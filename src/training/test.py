    ...
    try:
        ...
        try:
            ...
            if isinstance(loaded_model, NeuralAudioEncoding):
                ...
                print("Warning: Could not determine how to load the model. Using uninitialized model.")
...
def test_single_file(model, file_path, config, device, output_dir=None, visualize=False, play=False):
    ...
    if config["data"].get("transform"):
        transform = transform_audio(
            config["data"]["transform"]["min_max"],
            config["data"]["transform"]["softmax"]
        )
...
def test_dataset(model, config, device, output_dir=None, max_samples=None):
    ...
    if config["data"].get("transform"):
        test_dataset.transform = transform_audio(
            config["data"]["transform"]["min_max"], 
            config["data"]["transform"]["softmax"]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Test Neural Audio Modulation model on dataset or single file")
    parser.add_argument("--model_path", type=str, required=True, 
                        help="Path to saved model")
    parser.add_argument("--file", type=str, 
                        help="Path to a single audio file for testing")
    ...
    parser.add_argument("--visualize", action="store_true", 
                        help="Visualize input and output for single file mode")
    parser.add_argument("--play", action="store_true",
                        help="Play audio for single file mode")
    ...
    if args.file:
        # Single file mode
        test_single_file(model, args.file, config, device, args.output_dir, args.visualize, args.play)
    else:
        # Dataset mode
